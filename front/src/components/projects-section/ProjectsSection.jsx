...
const ProjectsSection = () => {
   ...
   return (
      <section id="projects" className="py-24 px-6 md:px-20 bg-transparent text-white">
         <div ref={sectionRef} className="max-w-7xl mx-auto">
            {/* Header */}
            <div className="text-center mb-16">
               <h2 className="text-4xl md:text-5xl font-bold text-[#FEBE0D]">Projects</h2>
               <p className="text-gray-300 max-w-2xl mx-auto mt-4">
                  A collection of my featured works, blending design precision with technical depth.
               </p>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
               {projects.map((project, index) => (
                  ...
                  >
                     ...
                     {/* Content */}
                     <div className="p-6">
                        <h3 className="text-xl font-semibold text-[#FEBE0D] mb-2">{project.title}</h3>
                        <p className="text-gray-300 text-sm leading-relaxed mb-4">{project.description}</p>

                        {/* Tech Stack */}
                        <div className="flex flex-wrap gap-2">
                           {project.tech.map((tech, i) => (
                              <span key={i} className="text-xs bg-[#FEBE0D]/20 text-[#FEBE0D] px-3 py-1 rounded-full">
                                 {tech}
                              </span>
                           ))}
                        </div>
